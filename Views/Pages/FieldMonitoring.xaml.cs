...
namespace SystemMonitoring.Views.Pages
{
	public partial class FieldMonitoring : Page
	{
		...
		public FieldMonitoring(Seed selectedSeed)
		{
			InitializeComponent();
			...
			CbCulture.ItemsSource = u519667_monitoringEntities.gc().Cultures.GroupBy(x => x.Name).Select(x => x.Key).ToList();
		...
		private void SaveToExcel_Click(object sender, RoutedEventArgs e)
		{
			...
			var field = u519667_monitoringEntities.gc().Fields.Single(x => x.ID == Db.SelectSeeding.IDField);
		...
		{
			var culture = u519667_monitoringEntities.gc().Cultures.Single(x => x.Name == CbCulture.SelectedItem.ToString() && x.Status == Status.Content.ToString());
			var recommendation = "";
			...
			else if (Convert.ToDouble(sensor.Magnesium) > _magnMax) recommendation += $"Магний в избыткена {_magnMid}мг/кг\n";
			else if (Convert.ToDouble(sensor.Magnesium) < _magnMin) recommendation += $"Рекомендуется внести магний на {_magnMid}мг/кг\n";
		...
		private void Culture_SelectionChanged(object sender, SelectionChangedEventArgs e)
		{
			if (!start)
			{
				_currentSeed.IDCulture = u519667_monitoringEntities.gc().Cultures.Single(x => x.Name == CbCulture.SelectedItem.ToString() && x.Period.Contains("00")).ID;
				_currentSeed.Culture = u519667_monitoringEntities.gc().Cultures.Single(x => x.Name == CbCulture.SelectedItem.ToString() && x.Period.Contains("00"));
				CbCulture.IsEnabled = false;
				_currentSeed.IDField = u519667_monitoringEntities.gc().Fields.Single(x => x.ID == _currentSeed.IDField).ID;
				_currentSeed.Field = u519667_monitoringEntities.gc().Fields.Single(x => x.ID == _currentSeed.IDField);
				_currentSeed.ID = Db.SelectSeeding.ID;
				var a1 = u519667_monitoringEntities.gc().Fields.Single(x => x.ID == Db.SelectSeeding.IDField);
				var a2 = u519667_monitoringEntities.gc().Cultures.Single(x => x.Name == CbCulture.SelectedItem.ToString() && x.Period.Contains("00"));
				...
				u519667_monitoringEntities.gc().Seeds.Add(seeding);
		...
		//{
		//	_currentSeed.Date = (sender as DatePicker).SelectedDate;
		//	u519667_monitoringEntities.gc().Seeds.Add(_currentSeed);
		//	u519667_monitoringEntities.gc().SaveChanges();
		//}
		...
		{
			int max = 0;
			var culture = u519667_monitoringEntities.gc().Cultures.Where(x => x.Name == CbCulture.SelectedItem.ToString() && x.Status == Status.Content.ToString()).ToList().Single();
