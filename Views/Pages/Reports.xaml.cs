...
namespace SystemMonitoring.Pages
{
	public partial class Reports : Page
	{
		...
		private void FileSearch(string path)
		{
			...
			{
				var listExcel = excelFiles.Select(file => new Models.FileInfo { FileName = file.Split('\\').Last(), Path = file }).ToList();
				DgExcelFiles.ItemsSource = listExcel;
			}
			//Word
			if (wordFiles.Count <= 0) return;
			var listWord = wordFiles.Select(file => new Models.FileInfo { FileName = file.Split('\\').Last(), Path = file }).ToList();
		...
		private void BtnOpenExcel_OnClick(object sender, RoutedEventArgs e)
		{
			...
			exApp.Workbooks.Open(((sender as Button)?.DataContext as Models.FileInfo)?.Path);
		}
		private void BtnOpenWord_OnClick(object sender, RoutedEventArgs e)
		{
			var wdApp = new Word.Application { Visible = true };
			wdApp.Documents.Open(((sender as Button)?.DataContext as Models.FileInfo)?.Path);
		}
	}
}